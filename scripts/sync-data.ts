...
import { fetchAndStoreAllHours, fetchAndStoreHour, cleanupOldData } from '@/lib/services/windborne';
  ...
  try {
    if (command === 'all') {
      ...
      result.hourResults.forEach(({ hour, count }) => {
        console.log(`  Hour ${hour.toString().padStart(2, '0')}: ${count} records`);
      });
      ...
      console.log('  npm run sync cleanup [days] - Cleanup data older than N days (default: 7)');
      process.exit(1);
    }
  } catch (error) {
